...
#### 限制调整
很多情况下LIMIT语句还是需要执行整个查询语句，然后再返回部分结果。
开启optimize后，当使用limit时，可以对元数据进行抽样。缺点是有可能输入中的有些数据永远不会被处理到。
``` shell
<property>
...
a vcore & b memory
每台机器： n executor = 1/n * a cores = 1/n * b  memory
4 vcore & 8g memory
